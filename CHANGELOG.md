...
## v1.0.27 (2026-07-01)：Windows 原生更新助手构建修复目标版
...
## v1.0.26 (2026-07-01)：Windows 原生更新助手构建修复基线版

### 问题修复

* 修复 Windows 原生更新助手包含 Windows SDK 头文件顺序导致 release 构建失败的问题。
* 继续使用原生 `SpringNoteUpdater.exe` 负责等待旧进程、安装和重启。
* 作为 1.0.27 自动更新目标测试的安装基线版本。
...
* Windows 自动更新不再依赖 PowerShell 脚本负责安装后重启。
* 新增原生 `SpringNoteUpdater.exe`，由它等待旧进程退出、运行安装器、写入更新日志并启动新版 SpringNote。
* 作为 1.0.25 自动更新目标测试的安装基线版本。

## v1.0.23 (2026-07-01)：Windows 安装后重启目标验证版

### 验证

* 用于验证 1.0.22 到 1.0.23 的 Windows 应用内更新、静默安装和安装后自动重启链路。
* 如果仍未自动启动，可用本次新增的 helper 日志定位安装器退出码和重启兜底路径。
...
## v1.0.20 (2026-07-01)：Windows 更新退出修复验证基线版

### 问题修复

* Windows 应用内更新不再让 Inno Setup 接管关闭 SpringNote。
* 下载和校验完成后，SpringNote 会先通过原生退出通道关闭自身，再由外部安装 helper 等待旧进程结束、静默安装并重启新版。
* 作为 1.0.21 自动更新目标测试的安装基线版本。

## v1.0.19 (2026-07-01)：自动更新重启目标验证版
...
## v1.0.18 (2026-07-01)：WinSparkle 签名修复验证基线版
...
## v1.0.17 (2026-07-01)：自动更新重启目标验证版

### 验证

* 用于验证 1.0.16 到 1.0.17 的 Windows 更新签名、静默安装和自动重启链路。
* 用于验证 macOS 自定义更新界面和自动重启替换体验。

## v1.0.16 (2026-07-01)：Windows 签名工具修复验证基线版

### 验证

* 修复 Windows 更新签名工具不能作为 global executable 运行的问题。
* 作为 1.0.17 自动更新目标测试的安装基线版本。

## v1.0.15 (2026-07-01)：自动更新重启目标验证版

### 验证

* 用于验证 1.0.14 到 1.0.15 的 Windows 更新签名、静默安装和自动重启链路。
* 用于验证 macOS 自定义更新界面和自动重启替换体验。

## v1.0.14 (2026-07-01)：Windows 签名修复验证基线版

### 验证

* 修复 Windows 更新签名步骤使用 Dart global executable 导致 release 构建失败的问题。
* 作为 1.0.15 自动更新目标测试的安装基线版本。

## v1.0.13 (2026-07-01)：自动更新重启目标验证版

### 验证

* 用于验证 1.0.12 到 1.0.13 的 Windows 静默安装后自动重启链路。
* 用于验证 macOS 自定义更新界面、下载进度、安装替换和重启体验。

## v1.0.12 (2026-07-01)：自动更新重启验证基线版

### 验证

* 用于验证新版 macOS 自定义更新界面和 Windows Restart Manager 重启链路。
* 作为 1.0.13 自动更新目标测试的安装基线版本。

## v1.0.11 (2026-07-01)：GitHub API 更新目标验证版
